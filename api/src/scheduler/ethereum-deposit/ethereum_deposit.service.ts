...
// Ethreum Address List
  ...
  async readBlockNumber() {
    const currentBlockData = await this.CommonCodeRepository.find({ where: { code_index: 'ethereum_current_block' } });
  ...
  async checkSingleBlock(blockNumber) {
    try {
      ...
      const filteredAddressesERC20 = await this.getEthereumAddressERC20(erc20_txs);
      this.logger.log(`Number of interested vanity address [ERC20]: ${filteredAddressesERC20.length}`);
