...
export class EtherController {
  ...
  @ApiOperation({
    summary: '현재의 블럭번호 조회',
    description: '현재의 블럭번호를 조회할 수 있다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Token 전송 Gas Price 계산할 수 있다. 전송속도 speed (0.5 ~ 4.0) [slow:0.7, normal:1, fast:2, very fast:4]',
  })
  ...
  async getTransferGasPrice(@Req() req, @Body() getTransferGasPriceDto: GetTransferGasPriceDto) {
    try {
      ...
      const gasPrice = await this.etherService.getTransferGasPrice(
        getTransferGasPriceDto.speed,
        getTransferGasPriceDto.toAddress,
      );
      this.logger.log(req.url + ' => ' + gasPrice);
  ...
  @ApiOperation({
    ...
    description:
      'Base Token 전송할 수 있다. 전송속도 speed (0.5 ~ 4.0) [slow:0.7, normal:1, fast:2, very fast:4]',
  })
  @ApiCreatedResponse({ description: 'Token 전송 Gas Price', type: ResponseGasPriceDto })
