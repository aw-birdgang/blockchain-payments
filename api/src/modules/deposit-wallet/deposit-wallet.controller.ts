...
export class DepositWalletController {
  constructor(private readonly clientWalletService: DepositWalletService) {}

  private readonly logger = new Logger(DepositWalletController.name);

  @Post()
  @ApiOperation({
    summary: '클라이언트 지갑 생성 API',
    description: '클라이언트 지갑을 생성 한다.',
  })
  @ApiCreatedResponse({ description: '클라이언트 를 생성 한다.', type: DepositWallet })
  async create(@Body() clientRequestCreateDto: ClientWalletRequestCreateDto, @Res() res: Response) {
